(function(jQuery) {
    ...
    jQuery.fn.stickyNotes.prepareContainer = function(container) {
        ...
        if (jQuery.fn.stickyNotes.options.controls) {
            jQuery("#sticky-container").html('<button id="add_note">添加便签</button>');
    ...
    jQuery.fn.stickyNotes.createNote = function() {
        ...
        var _div_delete = $(document.createElement('div')).addClass('jSticky-delete').click(function() {
            jQuery.fn.stickyNotes.deleteNote(this);
        });
        var _div_wrap = $(document.createElement('div')).css({
            'position' : 'absolute',
            'top' : pos_x,
            'left' : pos_y,
            'float' : 'left'
        }).attr("id", "note-" + note_id).append(_div_background).append(_div_note).append(_div_delete);
    ...
    jQuery.fn.stickyNotes.stopEditing = function(note_id) {
        ...
        var _p_note_text = $(document.createElement('p')).attr("id", "p-note-" + note_id).html(note.text);
        $("#note-" + note_id).find('textarea').replaceWith(_p_note_text);
        $("#p-note-" + note_id).click(function() {
    ...
    jQuery.fn.stickyNotes.editNote = function(paragraph) {
        ...
        var textarea = $(document.createElement('textarea')).attr("id", "textarea-note-" + note_id).val(
                $("#note-" + note_id).find('p').html());
    ...
    jQuery.fn.stickyNotes.renderNote = function(note) {
        ...
        var _div_wrap = $(document.createElement('div')).css({
            ...
            "width" : note.width,
            "height" : note.height
        }).attr("id", "note-" + note.id).append(_div_background).append(_div_note).append(_div_delete);
        _div_wrap.addClass('jSticky-medium');
        if (jQuery.fn.stickyNotes.options.resizable) {
            _div_wrap.resizable({
                stop : function(event, ui) {
    ...
    jQuery.fn.stickyNotes.createNoteBackground = function() {
        ...
        if (jQuery.browser.msie && jQuery.browser.version <= 6) {
            background = $(document.createElement('div'))
                    .addClass("background")
                    .html(
                            '<img src="/static/img/note/spacer.gif" class="stretch" style="margin-top:5px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\'/static/img/note/sticky-bg.png\',sizingMethod=\'scale\'");" alt="" />');
        } else {
            background = $(document.createElement('div')).addClass("background").html(
                    '<img src="/static/img/note/sticky-bg.png" class="stretch" style="margin-top:5px;" alt="" />');
        }
        return background;
