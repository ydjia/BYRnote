/**
 * @preserve FullCalendar v1.5.4 http://arshaw.com/fullcalendar/
 * 
...
(function($, undefined) {
    ...
    $.fn.fullCalendar = function(options) {
        ...
        options = $.extend(true, {}, defaults,
                (options.isRTL || options.isRTL === undefined && defaults.isRTL) ? rtlDefaults : {}, options);
        ...
        function render() {
            ...
            if (sections) {
                element = $("<table class='fc-header' style='width:100%'/>").append(
                        $("<tr/>").append(renderSection('left')).append(renderSection('center')).append(
                                renderSection('right')));
                return element;
            }
        }
                    ...
                    $.each(this.split(','),
                            function(j, buttonName) {
                                if (buttonName == 'title') {
                                    ...
                                    if (buttonClick) {
                                        var icon = options.theme ? smartProperty(options.buttonIcons, buttonName)
                                                : null; // why
                                        // are
                                        // we
                                        // using
                                        ...
                                        if (button) {
                                            button.click(function() {
                                                if (!button.hasClass(tm + '-state-disabled')) {
                                                    ...
                                                    function() {
                                                        button.not('.' + tm + '-state-active').not(
                                                                '.' + tm + '-state-disabled').addClass(
                                                                tm + '-state-down');
                                                    }).mouseup(function() {
                                                button.removeClass(tm + '-state-down');
                                                    ...
                                                    function() {
                                                        button.removeClass(tm + '-state-hover').removeClass(
                                                                tm + '-state-down');
                                                    }).appendTo(e);
                                            if (!prevButton) {
    ...
    function markFirstLast(e) {
        e.children().removeClass('fc-first fc-last').filter(':first-child').addClass('fc-first').end().filter(
                ':last-child').addClass('fc-last');
    }
    ...
    function getSkinCss(event, opt) {
        ...
        var backgroundColor = event.backgroundColor || eventColor || source.backgroundColor || sourceColor
                || opt('eventBackgroundColor') || optionColor;
        var borderColor = event.borderColor || eventColor || source.borderColor || sourceColor
                || opt('eventBorderColor') || optionColor;
    ...
    function BasicView(element, calendar, viewName) {
        ...
        function buildSkeleton(maxRowCnt, showNumbers) {
            ...
            for (i = 0; i < maxRowCnt; i++) {
                ...
                for (j = 0; j < colCnt; j++) {
                    s += "<td class='fc- " + contentClass + " fc-day" + (i * colCnt + j) + "'>" + // need
                    // fc-
                    // for
                    // setDayID
                    "<div>" + (showNumbers ? "<div class='fc-day-number'/>" : '') + "<div class='fc-day-content'>"
                            + "<div style='position:relative'>&nbsp;</div>" + "</div>" + "</div>" + "</td>";
                }
                s += "</tr>";
            }
            s += "</tbody>" + "</table>";
    ...
    function BasicEventRenderer() {
        ...
        function compileSegs(events) {
            var rowCnt = getRowCnt(), colCnt = getColCnt(), d1 = cloneDate(t.visStart), d2 = addDays(cloneDate(d1),
                    colCnt), visEventsEnds = $.map(events, exclEndDay), i, row, j, level, k, seg, segs = [];
            for (i = 0; i < rowCnt; i++) {
                ...
                start : function(ev, ui) {
                    ...
                    hoverListener.start(function(cell, origCell, rowDelta, colDelta) {
                        ...
                        if (cell) {
                            ...
                            renderDayOverlay(addDays(cloneDate(event.start), dayDelta), addDays(exclEndDay(event),
                                    dayDelta));
                        } else {
    ...
    function AgendaView(element, calendar, viewName) {
        ...
        function buildSkeleton() {
            ...
            s = "<table style='width:100%' class='fc-agenda-days fc-border-separate' cellspacing='0'>" + "<thead>"
                    + "<tr>" + "<th class='fc-agenda-axis " + headerClass + "'>&nbsp;</th>";
            for (i = 0; i < colCnt; i++) {
            ...
            s += "<th class='fc-agenda-gutter " + headerClass + "'>&nbsp;</th>" + "</tr>" + "</thead>" + "<tbody>"
                    + "<tr>" + "<th class='fc-agenda-axis " + headerClass + "'>&nbsp;</th>";
            for (i = 0; i < colCnt; i++) {
                ...
                // for
                // setDayID
                "<div>" + "<div class='fc-day-content'>" + "<div style='position:relative'>&nbsp;</div>" + "</div>"
                        + "</div>" + "</td>";
            }
            ...
            if (opt('allDaySlot')) {
                ...
                s = "<table style='width:100%' class='fc-agenda-allday' cellspacing='0'>" + "<tr>" + "<th class='"
                        + headerClass + " fc-agenda-axis'>" + opt('allDayText') + "</th>" + "<td>"
                        + "<div class='fc-day-content'><div style='position:relative'/></div>" + "</td>"
                        + "<th class='" + headerClass + " fc-agenda-gutter'>&nbsp;</th>" + "</tr>" + "</table>";
            ...
            slotScroller = $("<div style='position:absolute;width:100%;overflow-x:hidden;overflow-y:auto'/>").appendTo(
                    slotLayer);
            ...
            for (i = 0; d < maxd; i++) {
                ...
                s += "<tr class='fc-slot" + i + ' ' + (!minutes ? '' : 'fc-minor') + "'>"
                        ...
                        + ((!slotNormal || !minutes) ? formatDate(d, opt('axisFormat')) : '&nbsp;') + "</th>"
                        + "<td class='" + contentClass + "'>" + "<div style='position:relative'>&nbsp;</div>" + "</td>"
                        + "</tr>";
        ...
        function timePosition(day, time) { // both date objects. day holds
            ...
            var slotMinutes = opt('slotMinutes'), minutes = time.getHours() * 60 + time.getMinutes() - minMinute, slotI = Math
            ...
            if (ev.which == 1 && opt('selectable')) { // ev.which==1 means
                ...
                hoverListener.start(function(cell, origCell) {
                    ...
                    if (cell && cell.col == origCell.col && !cellIsAllDay(cell)) {
                        ...
                        dates = [ d1, addMinutes(cloneDate(d1), opt('slotMinutes')), d2,
    ...
    function AgendaEventRenderer() {
        ...
        function compileSlotSegs(events) {
            var colCnt = getColCnt(), minMinute = getMinMinute(), maxMinute = getMaxMinute(), d = addMinutes(
                    cloneDate(t.visStart), minMinute), visEventEnds = $.map(events, slotEventEnd), i, col, j, level, k, seg, segs = [];
            for (i = 0; i < colCnt; i++) {
        ...
        function slotSegHtml(event, seg) {
            ...
            html += " class='" + classes.join(' ') + "'" + " style='position:absolute;z-index:8;top:" + seg.top
                    + "px;left:" + seg.left + "px;" + skinCss + "'" + ">" + "<div class='fc-event-inner fc-event-skin'"
                    + skinCssAttr + ">" + "<div class='fc-event-head fc-event-skin'" + skinCssAttr + ">"
                    + "<div class='fc-event-time'>"
                    + htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) + "</div>" + "</div>"
                    + "<div class='fc-event-content'>" + "<div class='fc-event-title'>" + htmlEscape(event.title)
                    + "</div>" + "</div>" + "<div class='fc-event-bg'></div>" + "</div>"; // close
            // inner
            if (seg.isEnd && isEventResizable(event)) {
        ...
        function draggableDayEvent(event, eventElement, isStart) {
            ...
            eventElement.draggable({
                            ...
                            if (!cell.row) {
                                ...
                                if (isStart) {
                                    if (allDay) {
                                        ...
                                        // entire
                                        // width
                                        setOuterHeight(eventElement, slotHeight
                                                * Math.round((event.end ? ((event.end - event.start) / MINUTE_MS)
                ...
                stop : function(ev, ui) {
                    ...
                    trigger('eventDragStop', eventElement, event, ev, ui);
                    if (revert) {
                        // hasn't moved or is out of bounds (draggable has
                        // already
                        // reverted)
                        resetElement();
                        eventElement.css('filter', ''); // clear IE opacity
        ...
        function resizableSlotEvent(event, eventElement, timeElement) {
            ...
            eventElement.resizable({
                ...
                resize : function(ev, ui) {
                    ...
                    // account
                    slotDelta = Math.round((Math.max(slotHeight, eventElement.height()) - ui.originalSize.height)
                            / slotHeight);
                    if (slotDelta != prevSlotDelta) {
                        timeElement.text(formatDates(event.start, (!slotDelta && !event.end) ? null : // no
                        // change,
                        // so
                        // don't display
                        // time range
    ...
    function View(element, calendar, viewName) {
        ...
        function trigger(name, thisObj) {
            return calendar.trigger.apply(calendar, [ name, thisObj || t ].concat(Array.prototype.slice.call(arguments,
                    2), [ t ]));
        }
    ...
    function DayEventRenderer() {
        ...
        function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
            ...
            for (i = 0; i < segCnt; i++) {
                ...
                html += " class='" + classes.join(' ') + "'" + " style='position:absolute;z-index:8;left:" + left
                        + "px;" + skinCss + "'" + ">" + "<div" + " class='fc-event-inner fc-event-skin'"
                        + (skinCss ? " style='" + skinCss + "'" : '') + ">";
                if (true) {
                    html += "<span class='fc-event-time'>"
                            + htmlEscape(formatDates(event.start, event.end, opt('timeFormat'))) + "-</span>"
                            + "<span class='fc-event-time'>"
                            + htmlEscape(formatDates(event.end, event.start, opt('timeFormat'))) + "</span>";

                }
                html += "</br><span class='fc-event-title'>" + htmlEscape(event.title) + "</span>" + "</div>";
                if (seg.isEnd && isEventResizable(event)) {
                    html += "<div class='ui-resizable-handle ui-resizable-" + (rtl ? 'w' : 'e') + "'>"
                            + "&nbsp;&nbsp;&nbsp;" + // makes hit area a lot
                            // better for
                            // IE6/7
                            "</div>";
                }
        ...
        function getRowDivs() {
            ...
            var rowDivs = [];
            for (i = 0; i < rowCnt; i++) {
                rowDivs[i] = allDayRow(i).find('td:first div.fc-day-content > div'); // optimal
                // selector?
            }
            return rowDivs;
        }
    ...
    // this fix was only necessary for jQuery UI 1.8.16 (and jQuery 1.7 or
    // 1.7.1)
    // upgrading to jQuery UI 1.8.17 (and using either jQuery 1.7 or 1.7.1)
    // fixed
    // the problem
